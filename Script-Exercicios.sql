...
-- View Exercício 1
	create view projetos_andamento as select nome, descricao, status from projetos where status = 'Em Andamento';
select *from projetos_andamento;

-- Procedure Exercício 2
delimiter //
create procedure inserir_funcionario
(x_nome varchar(255), x_email varchar(255), x_senha varchar(45), x_id_departamento int, x_valor_hora decimal(15,2))
begin
	insert into funcionarios(nome, email, senha, id_departamento, valor_hora) 
	values (x_nome, x_email, x_senha, x_id_departamento, x_valor_hora);
end//
delimiter ;

call inserir_funcionario('João Pedro', 'joao@eletricista3', 'eletricista3', 6, 70.00);
select *from funcionarios f;

-- Function Exercício 3
delimiter //
create function calcular_horas_projetos(x_id_projeto int)
returns int
begin
	declare total_horas int;
	select sum(horas_trabalhadas) into total_horas from funcionarios_projetos fp where fp.id_projeto = x_id_projeto;
    return total_horas;
end//
delimiter ;

select calcular_horas_projetos(2);

-- Parte 2
delimiter //
create function calcular_custo_projeto(x_id_projeto int)
returns int
begin
	declare custo_total int;
    select sum(horas_trabalhadas * valor_hora) into custo_total from funcionarios_projetos fp where fp.id_projeto = x_id_projeto;
   return custo_total;
end//
delimiter ;

select calcular_custo_projeto(4);
