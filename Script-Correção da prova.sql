...
-- Exercício 1
select count(distinct fp.id_projeto) as projetos from departamentos d 
...
-- Exercício 3
select f.nome as funcionario, avg(fp.horas_trabalhadas) as media from funcionarios_projetos fp 
inner join funcionarios f on f.id_funcionario = fp.id_funcionario 
group by f.nome;
...
inner join funcionarios_projetos fp on f.id_funcionario = fp.id_funcionario
group by f.nome order by total_horas desc limit 1;
...
-- Exercício 6
update projetos set valor_total = 55000.00
where id_projeto = 1;
...
-- Exercício 8
select AVG(p.valor_total) as media from projetos p where p.status = 'Em Andamento'

-- Exercício 9
select f.nome as funcionario from funcionarios f where f.nome like '%1';

-- Exercício 10
select p.nome as projeto, p.valor_total from projetos p 
order by p.valor_total desc limit 1;

-- Exercício 11
select p.nome as projeto, f.nome as funcionario, sum(fp.horas_trabalhadas) as horas_trabalhadas from funcionarios_projetos fp 
inner join projetos p on p.id_projeto = fp.id_projeto 
inner join funcionarios f on f.id_funcionario = fp.id_funcionario 
group by p.nome, f.nome limit 5;

-- Exercício 12
select f.nome as funcionario, sum(fp.horas_trabalhadas)as total_horas from funcionarios f 
inner join funcionarios_projetos fp on f.id_funcionario = fp.id_funcionario
group by f.nome order by total_horas desc;

-- Exercício 13
select p.nome as projeto, count(fp.id_funcionario) as total_funcionario from projetos p 
left join funcionarios_projetos fp on p.id_projeto = fp.id_projeto 
group by p.nome;

-- Exercicio 14
select p.nome as projeto, f.nome as funcionario, sum(fp.valor_hora * fp.horas_trabalhadas) as valor_pago from funcionarios f 
inner join funcionarios_projetos fp on f.id_funcionario = fp.id_funcionario 
