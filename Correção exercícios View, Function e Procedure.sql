...
-- Exercício 1:
create view projetos_andamentos as select * from projetos p where status = 'Em Andamento';
select nome, descricao from projetos_andamentos;

-- Exercício 2 a):
delimiter //
create procedure inserir_funcionario(nome varchar(255), email varchar(255), id_departamento int, valor_hora decimal(15,2))
begin
	insert into funcionarios(nome, email, id_departamento, valor_hora)
	values (nome, email, id_departamento, valor_hora);
end//
delimiter ; 
call inserir_funcionario('Teste', 'teste@teste.com',1,95);
...
begin
	...
	from funcionarios_projetos fp where fp.id_projeto = projetos.id_projeto)
	where id_projeto = id;
end//
delimiter ;
...
select sum(horas_trabalhadas) into total_horas from funcionarios_projetos fp where id_projeto = id;
return total_horas;
end//
delimiter ;

select calcular_horas_projeto(3);
...
select sum(horas_trabalhadas * valor_hora) into valor_total from funcionarios_projetos fp where id_projeto = id;
return valor_total;
end//
delimiter ;

select calcular_custo_projeto(3); 
